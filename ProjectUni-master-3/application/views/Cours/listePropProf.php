...
<table class="table table-striped">
	...
	<tr>
		...
		<td><a class="col-12 badge badge-primary" href="<?php echo site_url('Main/note_prof/'.$value->id_eleve) ?>" >
		...
		<td>
			<?php if((!$value->profA)  && ($date<$value->date)) { ?>
			...
			<?php }
			else if($value->profA) {echo "Vous avez refusé ce cours"; }
...
<table class="table table-striped">
	...
	<?php foreach ($liste2 as $value) {?>
		<tr>
			...
			<td><?php echo $value->heure ?></td>
			<?php if(!$value->eleveA) { ?>
			<td>En attente de l'èlève</td>
			<?php } else echo "<td>Refusé par l'èlève</td>"; ?>
